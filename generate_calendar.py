...
<section class="panel"><h2 id="formTitle">Event aanmaken of wijzigen</h2><label class="req">Titel<input id="title" required></label><label class="req">Unieke ID<input id="id" required></label><label class="req">Categorie<select id="category" required>{options}</select></label><label class="req">Featured?<select id="featured" required><option value="false">Nee</option><option value="true">Ja</option></select></label><label class="req">Hele dag?<select id="allDay" onchange="toggleTime()" required><option value="false">Nee</option><option value="true">Ja</option></select></label><label class="req">Datum<input id="date" type="date" required></label><div id="timeFields"><label>Starttijd<input id="startTime" type="time" value="10:00"></label><label>Eindtijd<input id="endTime" type="time" value="11:00"></label></div><label class="req">Locatie<input id="location" required></label><label class="req">Registratie URL<input id="registration" required placeholder="https://..."></label><label>Banner URL<input id="bannerUrl" placeholder="https://..."></label><label class="req">Tags, gescheiden met komma's<input id="tags" required></label><label class="req">Beschrijving<textarea id="description" required></textarea></label><div class="button-row"><button onclick="saveEvent()">Event opslaan</button><button class="secondary" onclick="duplicateEvent()">Dupliceren</button><button class="danger" onclick="deleteEvent()">Verwijderen</button><button class="light" onclick="copyJson()">Kopieer JSON</button></div></section></div>
...
function requiredFields(){{return [title, window.id, category, featured, allDay, date, location, registration, tags, description]}}
...
function saveEvent(){{if(!validateForm())return false;const e=readForm();const idx=events.findIndex(x=>x.id===selectedId||x.id===e.id);if(idx>=0)events[idx]=e;else events.push(e);events.sort((a,b)=>(eventDate(a)||'').localeCompare(eventDate(b)||''));selectedId=e.id;render();previewEvent(e);previewJson();status('Event opgeslagen in lokale lijst. Klik op Publiceer naar GitHub om live te zetten.','okmsg');return true}}
