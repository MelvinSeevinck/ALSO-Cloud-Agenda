...
def build_ics(config, public_events):
    cal = config["calendar"]
    ...
    for event in sorted(public_events, key=parse_event_date):
        lines += [
            "BEGIN:VEVENT",
    ...
    <article class="event-card{featured}" data-category="{html.escape(event.get('category',''))}" data-search="{search}">
      ...
      <div class="event-content">
        ...
        <div class="event-actions">
          <a class="small-button tracked-link requires-identity" href="{html.escape(webcal_url)}" data-action="event_calendar_click" data-calendar-type="outlook" data-event-id="{event_id}" data-event-title="{title}">Outlook abonnement</a>
...
def build_public(config, public_events):
    cal = config["calendar"]
    ...
    if not config.get("settings", {}).get("showPastEvents", False):
        events = [e for e in public_events if parse_event_date(e).date() >= today]
    events = sorted(public_events, key=parse_event_date)
...
<p class="hint">De Outlook-knop gebruikt direct de <strong>webcal://</strong>-abonnementslink. Als Outlook alsnog een foutmelding geeft, kopieer dan onderstaande webcal-link en plak die in Outlook Desktop via Agenda toevoegen → Van internet/Abonneren via web.</p><p class="hint">{html.escape(config.get("tracking", {}).get("privacyNotice", ""))}</p><p>Webcal-abonnementslink:</p><p><code>{html.escape(webcal_url)}</code></p>
</section>
...
def main():
    ...
    public_events = [e for e in events if e.get('status', 'published') == 'published']
    ...
    (DOCS / "index.html").write_text(build_public(config, public_events), encoding="utf-8")
    ...
    print("Generated docs/events-admin.json")
